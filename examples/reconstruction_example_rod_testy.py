...
images = directory_images_importer(
    corrected_path,
    tomogram_cropped_max_y - tomogram_cropped_min_y,
    tomogram_cropped_max_x - tomogram_cropped_min_x,
)
...
magnified_voxel_volume_x_len = int(
    (x_max - x_min) * initial_voxel_size / magnified_voxel_size
)
magnified_voxel_volume_y_len = int(
    (y_max - y_min) * initial_voxel_size / magnified_voxel_size
)
magnified_voxel_volume_z_len = int(
    (z_max - z_min) * initial_voxel_size / magnified_voxel_size
)

magnified_vol_pos_x = (
    (x_min + x_max) / 2 - initial_voxel_volume_x_len / 2
) * initial_voxel_size
magnified_vol_pos_y = (
    (y_min + y_max) / 2 - initial_voxel_volume_y_len / 2
) * initial_voxel_size
magnified_vol_pos_z = (
    (z_min + z_max) / 2 - initial_voxel_volume_z_len / 2
) * initial_voxel_size
...
cropped_area_offset = (
    detector_row_vec * pixelSize * cropped_offset_from_center_y
    + detector_col_vec * pixelSize * cropped_offset_from_center_x
)
...
sourcePositions = np.ascontiguousarray(
    np.zeros((numAngles, 3)).astype(np.float32), dtype=np.float32
)
moduleCenters = np.ascontiguousarray(
    np.zeros((numAngles, 3)).astype(np.float32), dtype=np.float32
)
colVectors = np.ascontiguousarray(
    np.zeros((numAngles, 3)).astype(np.float32), dtype=np.float32
)
rowVectors = np.ascontiguousarray(
    np.zeros((numAngles, 3)).astype(np.float32), dtype=np.float32
)
...
for n in range(numAngles):
    phi = n * T_phi
    sourcePositions[n, :] = (
        rotate(rotation_axis, volume_to_source, phi) + magnified_area_to_volume
    )
    moduleCenters[n, :] = (
        rotate(rotation_axis, volume_to_detector, phi) + magnified_area_to_volume
    )
...
leapct.set_volume(
    magnified_voxel_volume_x_len,
    magnified_voxel_volume_y_len,
    magnified_voxel_volume_z_len,

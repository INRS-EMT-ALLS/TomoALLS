...
def black_box(params):
    ...
    for n in range(numAngles):
        ...
        sourcePositions[n, :] = (
            rotate(rotation_axis, center_source, phi) + center_volume
        )

        moduleCenters[n, :] = (
            rotate(rotation_axis, center_detector, phi) + center_volume
        )
    ...
    loss2 = normalize(
        clip_extremes(normalize(np.abs(normalize(img1) - normalize(img2)))),
    )
...
res = gp_minimize(
    black_box,  # the function to minimize
    [
        (1.3521324266376205 - 0.25, 1.3521324266376205 + 0.25),
        (1.9457712957749504 - 0.25, 1.9457712957749504 + 0.25),
        (-0.074152617285826 - 0.25, -0.074152617285826 + 0.25),
        (-0.05118715717364691 - 0.01, -0.05118715717364691 + 0.01),
        (0.005491918172085386 - 0.01, 0.005491918172085386 + 0.01),
    ],  # the bounds on each dimension of x,
    x0=[
        1.500644695072536,
        2.1031947017660872,
        -0.028829889565389233,
        -0.05690782511163468,
        0.004837986876485113,
    ],

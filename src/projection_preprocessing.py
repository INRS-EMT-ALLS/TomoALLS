...
def remove_streaks(image, max_iterations=20):
    ...
    min_x, max_x = (
        center_x - math.floor(center_mask_size / 2),
        center_x + math.floor(center_mask_size / 2),
    )
    min_y, max_y = (
        center_y - math.floor(center_mask_size / 2),
        center_y + math.floor(center_mask_size / 2),
    )
    ...
    while True:
        ...
        if x <= offset or height - offset <= x or y <= offset or (width - offset) <= y:
            break
        distribution_set = clip_extremes(
            fft_complex[x - offset : x + offset, y - offset : y + offset]
        )
        std = np.std(distribution_set)
        mean = np.real(np.mean(distribution_set))
        replacement_area = np.random.normal(
            loc=mean, scale=std, size=[replacement_range, replacement_range]
        )
        if (
            fft_complex[
                x - subimage_offset : x + subimage_offset + 1,
                y - subimage_offset : y + subimage_offset + 1,
            ].shape
            != replacement_area.shape
        ):
            break
        fft_complex[
            x - subimage_offset : x + subimage_offset + 1,
...
def generate_bad_pixel_map(
    path, height, width, dtype=np.uint16, kernel_size=33, ratio=0.3
):
    if not isfile(path):
...
def projection_correction(
    images, gain_map, offset_map, bad_pixel_map, min_x, max_x, min_y, max_y
):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=24) as exe:
        result = exe.map(
            single_image_correction, layers_list, gain_list, offeset_list, badpixel_list
        )
    ...
    for i in range(corrected_frames.shape[0]):
        ...
        img2 = np.round(
            normalize(
                median_filter(
                    clip_extremes(corrected_frames[0, min_x:max_x, min_y:max_y], 0.1), 3
                )
            )
        )
        img1 = np.round(
            normalize(
                median_filter(
                    clip_extremes(corrected_frames[i, min_x:max_x, min_y:max_y], 0.1), 3
                )
            )
        )
        ...
        if np.abs(dx) <= 10 and np.abs(dy) <= 10:
            corrected_list.append(
                cv2.warpAffine(
                    ...
                    M,
                    (
                        corrected_frames[i, :, :].shape[1],
                        corrected_frames[i, :, :].shape[0],
                    ),
                )
            )
